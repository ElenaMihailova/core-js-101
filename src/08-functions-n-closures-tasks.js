...
function logger(func, logFunc) {
  return (...arg) => {
    ...
    const functionParameters = JSON.stringify(arg).slice(1, -1);

    logFunc(`${functionName}(${functionParameters}) starts`);

    const result = func(...arg);

    logFunc(`${functionName}(${functionParameters}) ends`);
...
function partialUsingArguments(fn, ...args1) {
  return function applyPartialArguments(...args2) {
